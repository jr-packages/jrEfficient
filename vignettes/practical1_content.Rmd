...
1. Reproduce the timing results in chapter 2 using the `benchmark`
  function from the `rbenchmark` package. Remember to load the package using
```{r,echo=TRUE}
library("rbenchmark")
```
...
```
\noindent We can read the file the back in again using `read.csv`
```{r,cache=TRUE, echo = TRUE}
dd = read.csv("example.csv")
```
\noindent To get a baseline result, time the `read.csv` function call above.
```{r,cache=TRUE, echo = TRUE}
...
\noindent We will now look ways of speeding up this step.
  ...
  \noindent Then load the package in the usual way
    ...
    library("readr")
    ```
  \noindent This package contains the function `read\_csv`; a replacement function for `read.cvs`.
...
1. In this question, we'll compare matrices and data frames. Suppose we have a matrix, `d\_m`
```{r, echo = TRUE}
##For fast computers
...
```
\noindent Using the `benchmark` function, time these three examples.
