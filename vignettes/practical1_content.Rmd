...
and a data frame `d_df`:
    ...
    colnames(d_df) = paste0("c", 1:seq_len(d_df))
    ```
    ...
    Can you explain the result? Try varying the number of replications.
        ...
        ## 1. The very large difference when selecting columns and rows (in data
        ## frames) is because the data is stored in column major-order.
        ## Although the matrix is also stored in column major-order, because
        ## everything is the same type, we can efficiently select values.
